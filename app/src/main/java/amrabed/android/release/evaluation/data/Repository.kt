...
class Repository(context: Context) {
    ...
    fun loadTaskHistoryByDateRange(taskId: String, range: DateRange) =
        db?.history()?.taskHistoryByDateRange(taskId, range.start.millis, range.end.millis)
    ...
    fun addTask(task: Task) = AppDatabase.writeExecutor.execute { db!!.taskTable().insertTask(task) }
